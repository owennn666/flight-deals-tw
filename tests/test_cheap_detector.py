...
def _baseline(median=10000, mad=800, p10=8500, p05=8000, n=60, days=8):
...
def test_unreliable_baseline_returns_none():
    det = CheapFareDetector()
    # 樣本太少（<50）時上游 baseline() 本該回 None；這裡直接構造
    # sample_size=3 的 Baseline 確認 is_reliable 擋下判定，不成局。
    assert det.evaluate(_fare(5000), _baseline(n=3)) is None


def test_unreliable_baseline_single_day_returns_none():
    det = CheapFareDetector()
    # 樣本數夠（>=50）但只橫跨 1 天 → is_reliable 仍為 False，不成局。
    assert det.evaluate(_fare(5000), _baseline(n=60, days=1)) is None


def test_budget_carrier_normal_price_not_flagged():
    # 對應真實 bug 場景：廉航正常價落在歷史地板附近（等於 p10），不該被誤判成好康。
    det = CheapFareDetector()
    baseline = _baseline(median=6000, p10=5800, p05=5500, n=60)
